...
/** Уровни, подходящие для изучения при заданном целевом уровне (свой и ниже + следующий). */
...
/**
 * Резервная оценка уровня по частотному рангу — для слов, которых нет
 * в профилях CEFR-J/Octanove.
 */
export function levelFromFrequency(rank: number | null | undefined): CefrLevel {
  if (rank == null) return 'C1';
  if (rank <= 600) return 'A1';
  if (rank <= 1600) return 'A2';
  if (rank <= 3200) return 'B1';
  if (rank <= 5500) return 'B2';
  if (rank <= 8500) return 'C1';
  return 'C2';
}
...
/**
 * Служебные части речи: тренировать перевод «the» или «of» бессмысленно,
 * поэтому такие слова помечаются и по умолчанию не попадают в пуллы.
 */
...
export function isFunctionWord(text: string, pos: string | null): boolean {
  if (FUNCTION_WORDS.has(text)) return true;
