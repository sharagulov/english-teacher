...
/** Записывает расход одного вызова модели в профиль пользователя и дневную статистику. */
...
/** Сводка расхода на ИИ для экрана профиля. */
export async function getAiUsageOverview(userId: string) {
  ...
  return {
    ...
    pricing,
    /** Мягкая шкала для индикатора «за месяц» — не лимит, а ориентир. */
    monthReferenceUsd: 2,
