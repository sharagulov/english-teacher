...
import { AI_TASK_LABELS, GRAMMAR_TOPICS, SCHEMAS, TUTOR_SYSTEM, ask, chatReplySchema, clozeSchema, grammarQuizSchema, gradingSchema, sentenceTaskSchema, wordDeepDiveSchema, writingPromptSchema, type AiTaskType, } from '../lib/ai.js';
    ...
    switch (input.type) {
        ...
        case 'sentence_ru_en': {
            ...
            const task = await ask({
                system: TUTOR_SYSTEM,
                user: `Составь одно предложение для перевода ${toRussian ? 'с английского на русский' : 'с русского на английский'}. ` +
        ...
        case 'grammar_quiz': {
            ...
            const task = await ask({
                system: TUTOR_SYSTEM,
                user: `Составь один вопрос с четырьмя вариантами ответа по теме «${topic}» для уровня ${level}. ` +
            ...
            if (task.correctIndex >= task.options.length)
                task.correctIndex = 0;
            payload = { ...task };
        ...
        case 'cloze': {
            ...
            const task = await ask({
                system: TUTOR_SYSTEM,
                user: `Составь предложение уровня ${level} с одним пропуском, обозначенным как «___». ` +
                    (target
        ...
        case 'listening': {
            const task = await ask({
                system: TUTOR_SYSTEM,
                user: `Составь одну английскую фразу уровня ${level} для диктанта на слух: 6–14 слов, ` +
        ...
        case 'writing': {
            const task = await ask({
                system: TUTOR_SYSTEM,
                user: `Придумай короткое письменное задание для студента уровня ${level}. ` +
                    (topic ? `Тема: ${topic}. ` : '') +
        ...
        case 'word_deep_dive': {
            ...
            const task = await ask({
                system: TUTOR_SYSTEM,
                user: `Разбери английское слово «${word.text}» для студента уровня ${level}. ` +
...
export async function submitTask(userId: string, taskId: string, answer: string): Promise<SubmitTaskResult> {
    ...
    if (type === 'grammar_quiz') {
        ...
        const grading = await ask({
            system: TUTOR_SYSTEM,
            user: `Проверь работу студента уровня ${task.level}.\n\n${description}\n\n` +
