...
function parseArgs(argv: string[]): Options {
    const options: Options = { limit: 12000, refresh: false, minScore: 40 };
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === '--refresh')
            ...
            options.minScore = Number(argv[++i]) ?? options.minScore;
    }
    return options;
}
...
function resolveBucket(entry: TranslationEntry, preferredPos: string | null): PosBucket {
    ...
    if (matching.translations.length === 1 && matching !== best) {
        ...
        for (const translation of best.translations) {
            if (merged.length >= 5)
                break;
            if (!merged.some((t) => normalize(t) === normalize(translation)))
...
function cleanTranslations(rawList: string, englishWord: string, max = 6): string[] {
    ...
    for (const piece of pieces) {
        ...
        if (value.length > 48)
            continue;
        if (normalize(value) === normalize(englishWord))
            continue;
        if (/^[А-ЯЁ]/.test(value) && /^[a-z]/.test(englishWord))
            continue;
...
async function loadTranslations(refresh: boolean): Promise<Map<string, TranslationEntry>> {
    ...
    for (const row of senseRows) {
        ...
        for (const translation of translations) {
            if (bucket.translations.length >= 6)
                break;
        ...
        if (!sense || sense.length > 90 || bucket.senses.length >= 4)
            continue;
        if (bucket.senses.some((s) => s.sense.toLowerCase() === sense.toLowerCase()))
            continue;
...
async function main() {
    ...
    for (const [word] of [...frequency.entries()].sort((a, b) => a[1] - b[1]))
        push(word);
    for (const word of [...cefr.keys()].sort())
        push(word);
    ...
    log(`  → пропущено: нет перевода ${skipped.noTranslation.toLocaleString('ru')}, слабый score ${skipped.lowScore.toLocaleString('ru')}, имена собственные ${skipped.properNoun.toLocaleString('ru')}, неподходящая форма ${skipped.badShape.toLocaleString('ru')}`);
    log('│');
    log('├─ 3/4 Запись в базу');
    const existing = new Set((await prisma.word.findMany({ where: { ownerId: null }, select: { text: true } })).map((w) => w.text));
