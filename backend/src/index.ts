...
const app = Fastify({
  logger: env.isProd
    ...
    : {
        level: 'info',
        transport: undefined,
        // В разработке ужимаем лог до самого нужного.
        serializers: {
...
app.setErrorHandler((rawError, request, reply) => {
  ...
  // Ошибки валидации параметров — это ответ клиенту, а не сбой сервера.
  ...
  // Записи, которых нет (Prisma P2025), — это 404 для клиента.
  if (error.code === 'P2025') {
...
/*
 * В продакшене API и клиент живут на одном порту: собранный фронтенд отдаётся
 * статикой, а все не-API маршруты возвращают index.html, чтобы работали
 * прямые ссылки вида /practice/session/... при обновлении страницы.
 */
