/**
 * Награды за уровень. Очки рейтинга нельзя потратить, поэтому всё, что раньше
 * покупалось, теперь открывается ростом уровня — купить или пропустить нельзя.
 */
...
const rewardRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/', async (request) => {
    ...
    const items = levelRewards().map((item) => ({
      ...
      /** Для заморозок это текущий запас, для остального — признак открытости. */
