...
const statsRoutes: FastifyPluginAsync = async (app) => {
    ...
    app.get('/overview', async (request) => {
        ...
        const now = new Date();
        const [byStatus, attemptsAgg, correctCount, pools, perfectPools, todayStat, dueNow, dueTomorrow, dueWeek, aiCount, bestStreak, totalWords] = await Promise.all([
            ...
            prisma.userWord.count({
                where: { userId, isIgnored: false, dueAt: { gt: now, lte: new Date(now.getTime() + 86400000) } },
            }),
            prisma.userWord.count({
                where: { userId, isIgnored: false, dueAt: { gt: now, lte: new Date(now.getTime() + 7 * 86400000) } },
            }),
    ...
    app.get('/breakdown', async (request) => {
        ...
        const [byLevel, byMode, byMatchType, byTopic, hourly] = await Promise.all([
          ...
          WHERE uw.userId = ?
          GROUP BY w.level, uw.status`, userId),
            prisma.$queryRawUnsafe<{
                mode: string;
                attempts: bigint;
                correct: bigint;
           ...
           FROM Attempt WHERE userId = ? GROUP BY mode`, userId),
            prisma.$queryRawUnsafe<{
                matchType: string;
                count: bigint;
            }[]>(`SELECT matchType, COUNT(*) AS count FROM Attempt WHERE userId = ? GROUP BY matchType`, userId),
            prisma.$queryRawUnsafe<{
                topic: string;
                attempts: bigint;
                correct: bigint;
          ...
          LIMIT 20`, userId),
            prisma.$queryRawUnsafe<{
                ...
                COUNT(*) AS attempts,
                SUM(CASE WHEN isCorrect THEN 1 ELSE 0 END) AS correct
           FROM Attempt WHERE userId = ? GROUP BY hour ORDER BY hour`, userId),
        ]);
