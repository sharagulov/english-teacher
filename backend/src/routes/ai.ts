...
const aiRoutes: FastifyPluginAsync = async (app) => {
  ...
  /** Какие тренажёры доступны и на что они рассчитаны. */
  app.get('/meta', async () => ({
  ...
  app.post('/chats', async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Некорректный сценарий' });
  ...
  app.post('/chats/:id/messages', async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'Пустое сообщение' });
