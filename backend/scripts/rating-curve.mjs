...
const levelFromPoints = (points) => {
    ...
    while (level < MAX_LEVEL && totalForLevel(level + 1) <= points)
        level++;
    while (level > 1 && totalForLevel(level) > points)
        level--;
    return level;
};
const POINTS_PER_ANSWER = 14;
const ANSWERS_PER_HOUR = 220;
const POINTS_PER_DAY = 1600;
...
for (const level of rows) {
    ...
    console.log(`${String(level).padStart(7)} | ${String(total).padStart(11)} | ${String(step).padStart(16)} | ` +
