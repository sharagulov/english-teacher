...
const query = (params: Record<string, string | number | boolean | string[] | undefined>): string => {
...
export const api = {
    ...
    auth: {
        ...
        update: (body: Partial<Pick<User, 'name' | 'cefrLevel' | 'dailyGoalWords' | 'typoTolerance' | 'soundEnabled' | 'autoAdvance' | 'showTranscript' | 'timezoneOffset'>>) => request<{
            user: User;
        }>('PATCH', '/auth/me', body),
    },
    practice: {
        ...
        answer: (id: string, body: {
            ...
            result: AnswerResult;
            state: PoolState;
        }>('POST', `/practice/pools/${id}/answer`, body),
        undo: (id: string, body: {
            wordId: number;
        }) => request<{
            state: PoolState;
            undo: {
                rating: Pick<User, 'points' | 'level' | 'progress'>;
            };
        }>('POST', `/practice/pools/${id}/undo`, body),
        choiceHint: (id: string, body: {
            wordId: number;
        }) => request<{
            state: PoolState;
            spend: {
        ...
        abandon: (id: string) => request<{
            ok: boolean;
        }>('POST', `/practice/pools/${id}/abandon`),
    },
    words: {
        list: (params: {
            search?: string;
        ...
        setFlags: (id: number, body: {
            isFavorite?: boolean;
            isIgnored?: boolean;
        }) => request<unknown>('PATCH', `/words/${id}/flags`, body),
        dislike: (id: number, body?: {
            level?: number;
        }) => request<{
            dislikeLevel: number;
        }>('POST', `/words/${id}/dislike`, body ?? {}),
        reset: (id: number) => request<{
            ok: boolean;
        }>('POST', `/words/${id}/reset`),
        addCustom: (body: {
            text: string;
        ...
        }>('GET', '/words/meta/topics'),
    },
    stats: {
        overview: () => request<StatsOverview>('GET', '/stats/overview'),
        daily: (days = 30) => request<{
            series: DailyPoint[];
        }>('GET', `/stats/daily?days=${days}`),
        words: (params: {
            status?: string;
            level?: CefrLevel;
            search?: string;
        ...
        }>('GET', `/stats/attempts${query(params)}`),
        transactions: (limit = 50) => request<{
            items: {
                id: number;
                amount: number;
                reason: string;
        ...
        }>('GET', `/stats/transactions?limit=${limit}`),
        achievements: () => request<{
            items: (import('./types').Achievement)[];
        }>('GET', '/stats/achievements'),
        pools: (limit = 30) => request<{
            items: {
                id: string;
                mode: PracticeMode;
                size: number;
    ...
    ai: {
        ...
        submit: (id: string, answer: string) => request<{
            result: AiResult;
        }>('POST', `/ai/tasks/${id}/submit`, { answer }),
        audioText: (id: string) => request<{
            text: string;
        }>('GET', `/ai/tasks/${id}/audio-text`),
        history: (limit = 30) => request<{
            items: {
                id: number;
                createdAt: string;
                score: number;
        ...
        sendMessage: (id: string, message: string) => request<{
            result: {
                reply: string;
        ...
        }>('POST', `/ai/chats/${id}/messages`, { message }),
        deleteChat: (id: string) => request<{
            ok: boolean;
        }>('DELETE', `/ai/chats/${id}`),
    },
};
