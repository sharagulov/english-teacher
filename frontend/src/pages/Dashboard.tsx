...
export function Dashboard() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-10">
      ...
      <section
        className="rounded-[28px] px-5 py-6 sm:px-8 sm:py-8"
        style={{ background: 'var(--dash-hero)', color: 'var(--dash-hero-ink)' }}
      >
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Link
            to="/practice"
            className="inline-flex h-14 shrink-0 items-center justify-center rounded-2xl bg-white px-8 text-[16px] font-medium text-[#141414] transition-opacity hover:opacity-90 active:scale-[0.98]"
          >
            Заниматься
          </Link>
        </div>
        ...
        <div className="mt-8">
          {daily.loading && series.length === 0 ? (
            <div className="flex h-[160px] items-center justify-center text-[13px] text-white/35">Загрузка</div>
          ) : (
            <AreaChart
              tone="hero"
              height={160}
              ...
              emptyLabel="здесь появится кривая занятий"
            />
          )}
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
        <ColorTile
          ...
          delta={review.dueNow > 0 ? `${formatNumber(review.dueTomorrow)} завтра` : undefined}
          caption="слов ждут повторения сегодня"
        />
        <ColorTile
          tone="b"
          value={String(user.dailyStreak)}
          delta={user.longestStreak > user.dailyStreak ? `рекорд ${user.longestStreak}` : user.dailyStreak > 0 ? 'рекорд держится' : undefined}
          caption="дней серии занятий подряд"
        />
        <ColorTile
          tone="c"
          value={formatPercent(answers.accuracy)}
          delta={answers.attempts > 0 ? plural(answers.attempts, 'ответ', 'ответа', 'ответов') : undefined}
          caption="точных ответов за всё время"
        />
        <ColorTile
          tone="d"
          value={formatNumber(words.learned)}
          delta={`${formatPercent(words.coverage)} словаря`}
          caption="слов уже в активном запасе"
        />
      </section>
...
function ColorTile({
  ...
  return (
    <div
      className="flex min-h-[200px] flex-col justify-between rounded-[28px] px-6 py-6 sm:min-h-[240px]"
      style={{ background: `var(--dash-${tone})`, color: `var(--dash-${tone}-ink)` }}
    >
      <div>
...
function weekOverWeek(series: DailyPoint[]): string | undefined {
  ...
  if (prev === 0) return 'больше, чем неделю назад';
  const change = Math.round(((last - prev) / prev) * 100);
  if (change === 0) return 'как на прошлой неделе';
