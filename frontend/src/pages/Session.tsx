...
import { MATCH_TYPE_LABELS, MODE_LABELS, PART_OF_SPEECH_LABELS, formatDuration, formatNumber, formatPercent, plural, splitAroundWord, } from '../lib/format';
...
export function Session() {
    ...
    useEffect(() => {
        ...
        api.practice
            ...
            .catch((cause: unknown) => {
            if (alive)
                setLoadError(cause instanceof ApiError ? cause.message : 'Пулл не найден');
        });
        return () => {
            alive = false;
        };
    ...
    const applyDislike = useCallback((wordId: number, level: number) => {
        setState((current) => {
            if (!current?.question || current.question.wordId !== wordId)
                return current;
        ...
        setResult((current) => {
            if (!current || current.word.id !== wordId)
                return current;
            return { ...current, wordProgress: { ...current.wordProgress, dislikeLevel: level } };
        });
    }, []);
    const submit = useCallback(async (given: string, options?: {
        gaveUp?: boolean;
    }) => {
        if (!poolId || !question || sending)
            return;
        const trimmed = given.trim();
        if (!options?.gaveUp && !trimmed)
            return;
        setSending(true);
        try {
            const response = await api.practice.answer(poolId, {
                ...
                answer: trimmed,
                responseMs: Math.min(600000, Date.now() - askedAt.current),
                hintsUsed: 0,
            ...
            setResult({
                ...
                wordProgress: {
                    ...
                    dislikeLevel: Math.max(response.result.wordProgress.dislikeLevel ?? 0, question.dislikeLevel ?? 0),
                },
            });
            ...
            if (rating.leveledUp) {
                notify({
                    ...
                    description: rating.freezesGranted > 0
    ...
    const undo = useCallback(async () => {
        if (!poolId || !question || undoing || sending)
            return;
        setUndoing(true);
        try {
    ...
    const buyHint = useCallback(async () => {
        if (!poolId || !question || hinting || sending || question.hintUsed)
            return;
    ...
    if (loadError) {
        return (<div className="mx-auto max-w-md px-4 py-20">
    ...
    return (<div className="flex min-h-dvh flex-col">
      ...
      <header className="border-line bg-surface/85 sticky top-0 z-20 border-b backdrop-blur-md">
        <div className="mx-auto flex h-14 max-w-3xl items-center gap-4 px-4">
          <button type="button" onClick={() => navigate('/practice')} className="text-faint hover:text-ink inline-flex items-center gap-1 text-[13px] transition-colors">
          ...
          <span className="hidden sm:inline">
            <RatingPoints amount={user?.points ?? 0} iconSize={13} iconClassName="text-faint" valueClassName="text-[13px] text-faint"/>
          </span>
        </div>
      </header>
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-4 py-10">
        <div className="mb-6 flex items-center gap-2">
          ...
          {question.partOfSpeech ? (<Badge>{PART_OF_SPEECH_LABELS[question.partOfSpeech] ?? question.partOfSpeech}</Badge>) : null}
          {question.isRetry ? <Badge tone="warning">повтор в пулле</Badge> : null}
          {result?.sessionStreak && result.sessionStreak > 2 ? (<Badge tone="success">серия {result.sessionStreak}</Badge>) : null}
        </div>

        <Prompt question={question} showDislike={question.direction !== 'audio_en'} onDislike={(level) => applyDislike(question.wordId, level)}/>

        {phase === 'question' ? (<QuestionForm question={question} answer={answer} setAnswer={setAnswer} onSubmit={(value) => void submit(value ?? answer)} onGiveUp={() => void submit(answer, { gaveUp: true })} onHint={() => void buyHint()} sending={sending} hinting={hinting} inputRef={inputRef}/>) : result ? (<Feedback result={result} question={question} onNext={advance} onUndo={() => void undo()} undoing={undoing}/>) : null}
      </div>
    </div>);
}
function Prompt({ question, showDislike, onDislike, }: {
    question: Question;
    showDislike: boolean;
    ...
    return (<div className="mb-8">
      {isAudio ? (<div className="flex flex-col items-start gap-3">
          <button type="button" onClick={() => speak(question.prompt)} className="border-line hover:border-line-strong bg-raised flex h-16 w-16 items-center justify-center rounded-2xl border transition-colors" aria-label="Прослушать слово">
          ...
          {question.transcription ? (<p className="text-faint mt-2 text-[15px]">{question.transcription}</p>) : null}
          {showActions ? (<div className="mt-3 flex items-center gap-2">
              {showListen ? (<button type="button" onClick={() => speak(question.prompt)} title="Прослушать" aria-label="Прослушать" className="text-faint hover:bg-sunken hover:text-ink inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition-colors duration-150">
                  <Volume2 size={17} strokeWidth={1.75} aria-hidden="true"/>
                </button>) : null}
              {showDislike ? (<DislikeButton wordId={question.wordId} level={question.dislikeLevel ?? 0} onChange={onDislike}/>) : null}
            </div>) : null}
        </>)}

      <p className="text-soft mt-4 text-[13px]">
        {question.choices && question.choices.length > 0 ? (question.direction === 'ru_en' ? ('Выберите английское слово') : ('Выберите перевод')) : (<>
...
function QuestionForm({ question, answer, setAnswer, onSubmit, onGiveUp, onHint, sending, hinting, inputRef, }: {
    question: Question;
    answer: string;
    ...
    return (<div>
      {choices && choices.length > 0 ? (<div className="space-y-4">
          <div className="grid gap-2.5 sm:grid-cols-2">
            {choices.map((choice, index) => (<button key={choice} type="button" disabled={sending || hinting} onClick={() => onSubmit(choice)} className="border-line bg-raised hover:border-line-strong flex items-center gap-3 rounded-xl border px-4 py-3.5 text-left text-sm transition-colors duration-150 disabled:opacity-50">
                <Kbd>{index + 1}</Kbd>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {!question.hintUsed && question.hintCost != null ? (<Button variant="secondary" size="md" disabled={sending || hinting || !question.canAffordHint} onClick={onHint} title={question.canAffordHint
                ...
                {!hinting && question.hintCost != null ? (<RatingPoints amount={question.hintCost} sign="−" iconClassName="text-faint" valueClassName="text-faint"/>) : null}
              </Button>) : null}
          </div>
          ...
          <div className="flex gap-2.5">
            <input ref={inputRef} value={answer} onChange={(event) => setAnswer(event.target.value)} autoComplete="off" autoCapitalize="off" spellCheck={false} lang={question.direction === 'ru_en' ? 'en' : 'ru'} placeholder={question.direction === 'ru_en' ? 'english word' : 'перевод'} className="border-line bg-raised text-ink placeholder:text-faint focus:border-ink h-14 flex-1 rounded-xl border px-4 text-[17px] transition-colors outline-none"/>
      ...
      {!choices || choices.length === 0 ? (<div className="mt-4 flex items-center gap-3">
...
function Feedback({ result, question, onNext, onUndo, undoing, }: {
    result: AnswerResult;
    question: Question;
    onNext: () => void;
    onUndo: () => void;
    ...
    const heading = question.direction === 'ru_en'
        ? `${question.prompt} — ${shownAnswer}`
        : `${result.word.text} — ${shownAnswer}`;
    const alsoFits = result.allAnswers.filter((item) => item !== shownAnswer);
    return (<div>
      <div className={cx('animate-rise rounded-2xl border p-5', correct ? 'border-success/30 bg-success-soft' : 'border-danger/30 bg-danger-soft')}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            {alsoFits.length > 0 ? (<p className="text-soft mt-1.5 text-[13px]">
                Также подходит: {alsoFits.join(', ')}
              </p>) : null}
            {result.word.gloss ? <p className="text-faint mt-2 text-[13px] italic">{result.word.gloss}</p> : null}
          </div>

          {result.reward.points > 0 ? (<div className="text-right">
              <RatingPoints amount={result.reward.points} sign="+" iconSize={18} valueClassName="text-[22px] font-semibold text-ink"/>
        ...
        {result.reward.breakdown.length > 0 ? (<ul className="border-line/60 mt-4 flex flex-wrap gap-x-4 gap-y-1 border-t pt-3">
            {result.reward.breakdown.map((item) => (<li key={item.label} className="text-soft text-[12px]">
                {item.label} <span className="text-ink font-medium">{item.value}</span>
              </li>))}
          </ul>) : null}

        {!correct ? (<p className="text-soft mt-4 text-[13px]">Слово останется в пулле и вернётся ещё раз.</p>) : null}
      </div>
      ...
      <div className="mt-6 flex items-center gap-4">
        ...
        <div className="ml-auto flex items-center gap-3">
          {showUndo ? (<button type="button" onClick={onUndo} disabled={undoing} className="text-faint hover:text-ink text-[12px] transition-colors disabled:opacity-50">
        ...
        {examples.map((example) => (<li key={example.text}>
            <div className="flex items-start gap-2.5">
              <p className="text-soft flex-1 text-[14px] leading-snug">
                {splitAroundWord(example.text, word.text).map((part, index) => part.match ? (<span key={index} className="text-ink font-semibold">
                      {part.text}
                    </span>) : (<span key={index}>{part.text}</span>))}
              </p>
              <div className="mt-0.5 flex shrink-0 items-center gap-1.5">
                {isAiGeneratedExample(example.source) ? <AiExampleIndicator /> : null}
                {speechSupported() ? (<button type="button" onClick={() => speak(example.text)} className="text-faint hover:text-ink transition-colors" aria-label="Прослушать пример">
...
function Senses({ senses }: {
    senses: AnswerResult['word']['senses'];
}) {
    if (senses.length === 0)
        return null;
    return (<Card className="mt-4">
      ...
      <ul className="space-y-2">
        {senses.slice(0, 4).map((sense) => (<li key={sense.sense} className="text-[13px]">
...
function Summary({ state, summary }: {
    state: PoolState;
    summary: AnswerResult['poolSummary'] | null;
}) {
    const navigate = useNavigate();
    ...
    return (<div className="mx-auto w-full max-w-lg px-4 py-16">
      ...
      <Card className="mt-7">
        <div className="grid grid-cols-2 gap-5">
          ...
          <Stat label="Очки рейтинга" value={<RatingPoints amount={summary?.points ?? pool.pointsEarned} sign="+"/>} tone="accent"/>
        </div>
      </Card>
