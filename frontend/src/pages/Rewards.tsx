...
export function Rewards() {
    ...
    if (rewards.loading && !rewards.data)
        return <Loading label="Смотрим награды"/>;
    if (rewards.error)
        return <ErrorNote message={rewards.error} onRetry={rewards.reload}/>;
    if (!rewards.data || !user)
        return null;
    ...
    return (<div>
      <PageHeader title="Награды" description="Очки рейтинга не тратятся. Всё, что есть в приложении, открывается ростом уровня." action={<RatingPoints amount={rewards.data.points} iconSize={15} valueClassName="text-[15px] font-semibold text-ink"/>}/>
      ...
      <Card className="mb-6">
        <div className="flex flex-wrap items-center gap-x-10 gap-y-5">
          ...
          <Stat label="Заморозки серии" value={`${streakFreezes} / ${maxStreakFreezes}`} hint={`выдаётся каждые ${freezeGrantEvery} уровней`} tone={streakFreezes > 0 ? 'accent' : undefined}/>

          <Stat label="Ближайшая награда" value={nextLevelItems[0] ? `${nextLevelItems[0].level} ур.` : '—'} hint={nextLevelItems[0]?.title ?? 'всё открыто'}/>
        </div>
      </Card>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (<Card key={item.code} className={cx('flex flex-col', !item.unlocked && 'opacity-60')}>
            <div className="flex items-start justify-between gap-3">
              <p className="text-ink text-sm font-medium">{item.title}</p>
              {item.unlocked ? (<Badge tone="success">{item.kind === 'freeze' ? `×${item.quantity}` : 'открыто'}</Badge>) : (<Badge>с {item.level} ур.</Badge>)}
            </div>
