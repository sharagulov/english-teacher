...
import { Badge, Button, Card, ErrorNote, Input, Loading, PageHeader, Progress, SectionTitle, Select, Stat, cx, } from '../components/ui';
...
export function Profile() {
    ...
    const goalOptions = GOALS.includes(user.dailyGoalWords)
        ? GOALS
        : [...GOALS, user.dailyGoalWords].sort((a, b) => a - b);
    const save = async (patch: Parameters<typeof updateSettings>[0], quiet = false) => {
        ...
        try {
            ...
            if (!quiet)
                notify({ title: 'Сохранено', tone: 'success' });
        }
        catch (cause) {
        ...
        <div className="space-y-5">
          ...
          <Card>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Select label="Ваш уровень английского" value={user.cefrLevel} onChange={(event) => void save({ cefrLevel: event.target.value as CefrLevel })}>
                {LEVELS.map((item) => (<option key={item.value} value={item.value}>
                    {item.label}
                  </option>))}
              </Select>
              <Select label="Дневная цель" value={String(user.dailyGoalWords)} onChange={(event) => void save({ dailyGoalWords: Number(event.target.value) })}>
                {goalOptions.map((value) => (<option key={value} value={value}>
            ...
            {name !== user.name ? (<Button variant="primary" className="mt-4" loading={saving} onClick={() => void save({ name })}>
                Сохранить имя
              </Button>) : null}
          ...
          <Card>
            ...
            <div className="space-y-1">
              <SwitchField label="Прощать опечатки" description="До двух опечаток в слове засчитываются как верный ответ (в коротких — не больше одной), но награда снижается." checked={user.typoTolerance} onChange={(value) => void save({ typoTolerance: value }, true)}/>
              <SwitchField label="Озвучка" description="Слова и реплики репетитора произносятся синтезатором браузера." checked={user.soundEnabled} onChange={(value) => void save({ soundEnabled: value }, true)}/>
              <SwitchField label="Показывать транскрипцию" description="Транскрипция выводится рядом со словом, если она известна." checked={user.showTranscript} onChange={(value) => void save({ showTranscript: value }, true)}/>
            </div>
          </Card>
          ...
          <Card>
            ...
            return (<button key={option.value} type="button" onClick={() => setTheme(option.value)} className={cx('flex-1 rounded-xl border p-3 text-left transition-colors duration-150', theme === option.value ? 'border-ink' : 'border-line hover:border-line-strong', locked && 'opacity-50')}>
          ...
          <Card>
            ...
            {health.data ? (<>
                ...
                <ul className="text-faint mt-3 space-y-1 text-[12px] leading-relaxed">
                  {health.data.attribution.map((line) => (<li key={line}>{line}</li>))}
                </ul>
              </>) : (<Loading label=""/>)}
          </Card>
        </div>
        ...
        <aside className="space-y-5">
          <Card>
            ...
            <div className="border-line mt-5 grid grid-cols-2 gap-4 border-t pt-5">
              ...
              <Stat label="До уровня выше" value={progress.isMax ? '—' : formatNumber(progress.pointsToNext)}/>
          ...
          <Card>
            ...
            {rewards.data ? (<ul className="space-y-2">
                ...
                .map((item) => (<li key={item.code} className="flex items-center justify-between gap-3 text-[13px]">
...
function AiUsageSection({ data, loading }: {
    data: AiUsageOverview | null;
    loading: boolean;
}) {
    if (loading && !data) {
        return (<Card>
    ...
    return (<Card>
      <SectionTitle title="Расход на нейросети" description={data.enabled && data.model
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Stat label="Всего потрачено" value={formatUsd(allTime.costUsd)} hint={`≈ ${formatRub(allTime.costRub)} · курс ${pricing.usdRubRate} ₽/$`} tone={allTime.costUsd > 0 ? 'accent' : undefined}/>
        <Stat label="За этот месяц" value={formatUsd(month.costUsd)} hint={`сегодня ${formatUsd(today.costUsd)} · ${formatNumber(allTime.requests)} ${plural(allTime.requests, 'запрос', 'запроса', 'запросов')}`}/>
      </div>
      ...
      {allTime.totalTokens === 0 ? (<p className="text-soft mt-4 text-[13px]">Пока нет данных — выполните задание ИИ или поговорите с репетитором.</p>) : null}
    </Card>);
}
