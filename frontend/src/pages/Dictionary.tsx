...
import { Badge, Button, Card, EmptyState, ErrorNote, Input, Loading, Modal, PageHeader, Select, Textarea, cx, } from '../components/ui';
import { ApiError, api } from '../lib/api';
import { PART_OF_SPEECH_LABELS, WORD_STATUS_LABELS, formatDateTime, formatNumber, formatPercent, formatRelative, formatResponseTime, splitAroundWord, } from '../lib/format';
...
export function Dictionary() {
    ...
    const words = useAsync(() => api.words.list({
        page,
        perPage: PER_PAGE,
    ...
    return (<div>
      <PageHeader title="Словарь" description="Общий словарь собран из открытых источников; прогресс по каждому слову — ваш личный." action={<Button variant="secondary" onClick={() => setAdding(true)}>
            Добавить своё слово
          </Button>}/>
      ...
      <Card className="mb-5">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Input label="Поиск" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="слово или перевод"/>
          <Select label="Уровень" value={level} onChange={(event) => {
          ...
          <Select label="Часть речи" value={partOfSpeech} onChange={(event) => {
            setPartOfSpeech(event.target.value);
            setPage(1);
        }}>
            <option value="">Любая</option>
            {meta.data?.partsOfSpeech.map((item) => (<option key={item.partOfSpeech} value={item.partOfSpeech}>
                {PART_OF_SPEECH_LABELS[item.partOfSpeech] ?? item.partOfSpeech} ({item.count})
              </option>))}
          </Select>
          <Select label="Тема" value={topic} onChange={(event) => {
            setTopic(event.target.value);
            setPage(1);
        }}>
            <option value="">Любая</option>
            {meta.data?.topics.map((item) => (<option key={item.topic} value={item.topic}>
        ...
        <div className="text-soft mt-4 flex items-center justify-between gap-3 text-[13px]">
          ...
          <Switch checked={onlyUnseen} aria-label="Только слова, которые я ещё не встречал" onChange={(value) => {
            setOnlyUnseen(value);
            setPage(1);
        }}/>
        </div>
      ...
      {words.loading && !words.data ? (<Loading label="Ищем слова"/>) : (words.data?.items.length ?? 0) === 0 ? (<Card>
          ...
          <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
            {words.data?.items.map((word) => (<WordCard key={word.id} word={word} onOpen={() => setOpenId(word.id)}/>))}
          </div>
      ...
      {openId !== null ? (<WordModal id={openId} aiEnabled={health.data?.aiEnabled ?? false} onClose={() => setOpenId(null)} onChanged={words.reload}/>) : null}
...
function WordCard({ word, onOpen }: {
    ...
    return (<button type="button" onClick={onOpen} className="border-line bg-raised hover:border-line-strong rounded-2xl border p-4 text-left transition-colors duration-150">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          ...
          {word.progress ? (<Badge tone={STATUS_TONE[word.progress.status]}>{WORD_STATUS_LABELS[word.progress.status]}</Badge>) : null}
        </div>
      </div>

      <p className="text-soft mt-2 line-clamp-2 text-[13px]">{word.translations.join(', ')}</p>

      {word.progress ? (<div className="bg-sunken mt-3 h-1 overflow-hidden rounded-full">
          <div className="bg-accent h-full rounded-full" style={{ width: `${word.progress.strength * 100}%` }}/>
        </div>) : null}
    </button>);
}
function WordModal({ id, aiEnabled, onClose, onChanged, }: {
    id: number;
    aiEnabled: boolean;
    onClose: () => void;
    onChanged: () => void;
    ...
    return (<Modal open onClose={onClose} wide>
      {detail.loading && !detail.data ? (<Loading label=""/>) : detail.error ? (<ErrorNote message={detail.error} onRetry={detail.reload}/>) : word ? (<div>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2.5">
                ...
                <DislikeButton wordId={id} level={progress?.dislikeLevel ?? 0} onChange={() => {
                detail.reload();
                onChanged();
            }}/>
              </div>
              <div className="mt-1.5 flex items-center gap-3">
                ...
                <button type="button" onClick={() => playPronunciation(word.text, word.audioUrl)} className="text-faint hover:text-ink text-[13px] transition-colors">
                  Прослушать
                </button>
              </div>
            </div>
            <div className="flex flex-wrap gap-1.5">
              <Badge>{word.level}</Badge>
              {word.partOfSpeech ? (<Badge>{PART_OF_SPEECH_LABELS[word.partOfSpeech] ?? word.partOfSpeech}</Badge>) : null}
          ...
          {word.examples.length > 0 ? (<div className="border-line mt-5 border-t pt-4">
              <p className="text-faint mb-2 text-[12px] font-medium tracking-wide uppercase">Примеры</p>
              <ul className="space-y-2.5">
                {word.examples.map((example) => (<li key={example.text}>
                    <div className="flex items-start gap-1.5">
                      ...
                      {isAiGeneratedExample(example.source) ? (<span className="mt-0.5">
                          <AiExampleIndicator />
                        </span>) : null}
                    </div>
                    {example.translation ? (<p className="text-faint mt-0.5 text-[13px]">{example.translation}</p>) : null}
                  </li>))}
              </ul>
            </div>) : null}

          {word.senses.length > 0 ? (<div className="border-line mt-5 border-t pt-4">
              <p className="text-faint mb-2 text-[12px] font-medium tracking-wide uppercase">Значения</p>
              <ul className="space-y-2">
                {word.senses.map((sense) => (<li key={sense.sense} className="text-[13px]">
          ...
          {progress ? (<div className="border-line mt-5 border-t pt-4">
            ...
            </div>) : (<p className="text-faint border-line mt-5 border-t pt-4 text-[13px]">
              Это слово вам ещё не попадалось.
            </p>)}

          
          {(detail.data?.recentAttempts.length ?? 0) > 0 ? (<div className="border-line mt-5 border-t pt-4">
              <p className="text-faint mb-2 text-[12px] font-medium tracking-wide uppercase">Последние ответы</p>
              <ul className="space-y-1.5">
                {detail.data?.recentAttempts.map((attempt, index) => (<li key={`${attempt.createdAt}-${index}`} className="flex items-center gap-3 text-[13px]">
          ...
          {dive ? (<div className="bg-sunken mt-5 rounded-xl p-4">
          ...
          <div className="border-line mt-5 flex flex-wrap gap-2 border-t pt-4">
            ...
            {!word.enriched ? (<Button size="sm" loading={enriching} onClick={enrich}>
                Дополнить из словаря
              </Button>) : null}
            {aiEnabled ? (<Button size="sm" loading={diving} onClick={deepDive}>
                Разбор от ИИ
              </Button>) : null}
            {progress ? (<Button size="sm" variant="ghost" onClick={reset}>
                Сбросить прогресс
              </Button>) : null}
          </div>
...
function AddWordModal({ open, onClose, onAdded }: {
    open: boolean;
    onClose: () => void;
    onAdded: () => void;
}) {
    ...
    const parsed = useMemo(() => translations
        .split(/[,;\n]/)
        .map((item) => item.trim())
    ...
    return (<Modal open={open} onClose={onClose} title="Своё слово">
      <div className="space-y-4">
        <Input label="Слово по-английски" value={text} onChange={(event) => setText(event.target.value)} placeholder="serendipity"/>
        <Textarea label="Переводы" rows={3} value={translations} onChange={(event) => setTranslations(event.target.value)} placeholder="счастливая случайность, интуитивная прозорливость" hint="Через запятую или с новой строки — любой из них будет считаться верным"/>
        ...
        <div className="flex gap-2">
          <Button variant="primary" loading={saving} disabled={!text.trim() || parsed.length === 0} onClick={save}>
            Добавить
          </Button>
