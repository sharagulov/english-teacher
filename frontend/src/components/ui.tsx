...
const BUTTON_BASE =
  'inline-flex items-center justify-center font-medium transition-[opacity,background-color,border-color,transform] duration-150 select-none active:scale-[0.98] disabled:cursor-not-allowed disabled:active:scale-100';
...
/** Ссылка, оформленная как кнопка: вкладывать <Link> внутрь <button> нельзя. */
...
const FIELD_BASE =
  'w-full bg-raised border border-line rounded-xl px-3.5 py-2.5 text-sm text-ink placeholder:text-faint transition-colors duration-150 focus:border-accent focus:outline-none disabled:opacity-50';
...
export function Card({
  ...
  return (
    <div className={cx('bg-raised border border-line rounded-2xl', padded && 'p-5', className)}>{children}</div>
  );
}
...
export function ErrorNote({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div className="bg-danger-soft flex items-center justify-between gap-4 rounded-xl px-4 py-3">
...
export function Loading({ label = 'Загрузка' }: { label?: string }) {
  return (
    <div className="text-faint flex items-center justify-center gap-2 py-16 text-sm">
      <Spinner />
      {label}
    </div>
  );
...
/** Клавиша в подсказках по горячим клавишам. */
export function Kbd({ children }: { children: ReactNode }) {
  return (
    <kbd className="border-line bg-sunken text-faint inline-flex h-5 min-w-5 items-center justify-center rounded border px-1.5 font-sans text-[11px] font-medium">
      {children}
    </kbd>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div
        ...
        aria-label={title}
        className={cx(
          'bg-raised border-line animate-rise relative max-h-[88vh] w-full overflow-y-auto rounded-t-3xl border p-6 shadow-card sm:rounded-2xl',
        ...
        {title ? (
          <div className="mb-4 flex items-start justify-between gap-4">
            ...
            <button
              ...
              onClick={onClose}
              aria-label="Закрыть"
              className="text-faint hover:text-ink -mt-1 -mr-1 h-8 w-8 rounded-lg transition-colors"
            >
              ✕
            </button>
          </div>
